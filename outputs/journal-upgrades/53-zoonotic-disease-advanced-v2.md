...
*DRAFT for author review — not for live publication without sign-off. All AACT-derived
trial counts (global/African/interventional zoonotic totals, the 4.33% baseline) are
regenerated live by `53-zoonotic-verify.py` (via `_aact_africa.py`) against the AACT
April-12-2026 snapshot; cited epidemiological/burden numerals (e.g. rabies deaths,
outbreak-year clustering, historical zoonoses percentages) are AS-CITED to their
published sources, not script-regenerated. According to PubMed metadata, every retained
PMID was verified by title/journal/pages match on 2026-07-09.*
