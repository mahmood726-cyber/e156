...
(function (global) {
   ...
   *   A stage with ≥2 exits FANS them out horizontally — side-by-side proportional sinks,
   *   each with a count + rotated reason label (the SVG auto-grows taller). 1 exit is the
   *   classic single downward arm (unchanged). Use the fan for itemised loss breakdowns
   *   (e.g. PRISMA exclusion reasons, CONSORT loss categories).
  ...
  function renderSankey(svgEl, stages, opts) {
    ...
    var k = stages.length;
    var fanActive = stages.some(function (s) { return s.exits && s.exits.length >= 2; });
    var W = o.width, H = fanActive ? Math.max(o.height, 392) : o.height, x0 = o.padL, x1 = W - o.padR, yTop = 84;
    ...
    for (i = 0; i < k - 1; i++) {
      ...
      var xSrc = nodeX(i) + nodeW;
      if (exits.length < 2) {
        var e0 = exits[0], eThk0 = t(e0.n); if (eThk0 < 0.5) eThk0 = 0.5;
        var xSink0 = xSrc + span * 0.46;
        var g0 = el('g', { tabindex: '0', role: 'listitem', 'aria-label': e0.label + ': ' + fmtN(e0.n) });
        fmtExit(g0, e0);
        ribbon(xSrc, wedgeTop, wedgeTop + eThk0, xSink0, sinkBase, sinkBase + eThk0, '0.34', g0);
        txt(xSink0, sinkBase + eThk0 + 13, '−' + fmtN(e0.n), { 'font-size': tok('--t-fs-annot', '11px'), fill: C.exit, 'text-anchor': 'middle' }, g0);
      } else {
        var m = exits.length, fanL = xSrc + span * 0.16, fanR = nodeX(i + 1) - 5;
        var slot = (fanR - fanL) / m, cum = 0, capMin = 3;
        // shared label baseline below the tallest cap → reasons read as a neat aligned row.
        // count + reason fold into ONE rotated label, so narrow slots never collide.
        var maxThk = Math.max.apply(null, exits.map(function (e) { return Math.max(t(e.n), capMin); }));
        var labelY = sinkBase + maxThk + 8;
        exits.forEach(function (e, idx) {
          ...
          el('rect', { x: xC - 1.6, y: sinkBase, width: 3.2, height: Math.max(capMin, eThk), fill: C.exit, 'fill-opacity': '0.85' }, g);
          var rl = txt(xC, labelY, '−' + fmtN(e.n) + '  ' + e.label, { 'font-size': tok('--t-fs-tick', '10.5px'), fill: C.exit, 'text-anchor': 'end' }, g);
