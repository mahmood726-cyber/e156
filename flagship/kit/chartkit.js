...
(function (global) {
  ...
  function renderSankey(svgEl, stages, opts) {
    ...
    var W = o.width, H = o.height, x0 = o.padL, x1 = W - o.padR, yTop = 84;
    var k = stages.length;
    ...
    for (i = 0; i < k - 1; i++) {
      ...
      var xSrc = nodeX(i) + nodeW, xSink = nodeX(i) + nodeW + span * 0.46;
      var cx = (xSrc + xSink) / 2, cum = 0, sinkY = sinkBase;
      exits.forEach(function (e) {
        var eThk = t(e.n); if (eThk < 0.5) return;
        var srcTop = wedgeTop + cum, srcBot = srcTop + eThk; cum += eThk;
        var sTop = sinkY, sBot = sinkY + eThk; sinkY = sBot + 6;
        var g = el('g', { tabindex: '0', role: 'listitem', 'aria-label': e.label + ': ' + fmtN(e.n) });
        addTip(g, '<b>' + e.label + '</b><br>−' + fmtN(e.n) + (o.unit ? ' ' + o.unit : ''));
        el('path', { d: 'M ' + xSrc + ' ' + srcTop +
          ' C ' + cx + ' ' + srcTop + ' ' + cx + ' ' + sTop + ' ' + xSink + ' ' + sTop +
          ' L ' + xSink + ' ' + sBot +
          ' C ' + cx + ' ' + sBot + ' ' + cx + ' ' + srcBot + ' ' + xSrc + ' ' + srcBot + ' Z',
          fill: C.exit, 'fill-opacity': '0.34' }, g);
        txt(xSink, sBot + 13, '−' + fmtN(e.n), { 'font-size': tok('--t-fs-annot', '11px'), fill: C.exit, 'text-anchor': 'middle' }, g);
      });
    }
